...
from mobileNetFeatureExtract3 import intermediate_output
    ...
    while True:
        ...
        for i in range(len(boxes)):
            ...
            if classes[i] == 1 and scores[i] > 0.3:  # Class 1 represents human
                ...
                personNumber+=1    
                value = intermediate_output(model,'roi.png')
        ...
        df = pd.DataFrame({'Person':names,
                   'Feature':pf})
        sns.set(style ="whitegrid") 
        ax = sns.stripplot(x='Person', y='Feature', jitter=True, data=df)
        plt.title('Graph') 
